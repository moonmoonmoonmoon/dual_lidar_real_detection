...
class LidarObjectDetector:
    ...
    def process_frame(self, frame_id: int, pcd: o3d.geometry.PointCloud,
                      pose_matrix: np.ndarray,timestamp: np.uint64) -> DetectionResult:
        """处理单帧点云数据"""
        try:
            ...
            # 3. ROI裁剪
            roi_start = time.time()
            roi_pcd = self.crop_roi(filtered_pcd, pose_matrix)
    ...
    def remove_ground(self, pcd: o3d.geometry.PointCloud) -> o3d.geometry.PointCloud:
        ...
        if self.first_frame:
            ...
            # 计算transformation matrix (只在第一次计算)
            if self.transform_matrix is None:
                self.transform_matrix = self.compute_transform_matrix(plane_model)
                print("Transformation matrix:", self.transform_matrix.tolist())
        ...
        # 应用变换
        transformed_non_ground = non_ground_pcd.transform(self.transform_matrix)
        ground_transform_time = time.time() - ground_transform_start_time
        print(f'Ground transform time: {ground_transform_time * 1000:.1f}ms')

        # 可视化地面检测结果
        if self.vis and self.config['visualization']['show_ground_removal']:
            ground_pcd = pcd.select_by_index(inliers)
            transformed_ground = ground_pcd.transform(self.transform_matrix)
            self.vis.visualize_ground(transformed_ground, transformed_non_ground)
        return transformed_non_ground
    ...
    def crop_roi(self, pcd: o3d.geometry.PointCloud,
                 pose_matrix: np.ndarray) -> o3d.geometry.PointCloud:
        """ROI区域裁剪"""
        box_size = np.array([
        ...
        # combined_matrix = pose_matrix
        lidar_position = combined_matrix[:3, 3]
