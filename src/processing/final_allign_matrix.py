...
def register_point_clouds(
    ...
    transformation_matrix = np.array([
        [-0.99891327, -0.04650292, -0.00312341, -2.97499893],
        [0.04660203, -0.9975914, -0.05137749, -0.6508804],
        [-0.00072668, -0.05146721, 0.99867442, 0.22458365],
        [0.0, 0.0, 0.0, 1.0]
    ])
    ...
    for i in range(num_initial_registration_loops):
        ...
        if transformation_matrix is not None:
            icp_result = refine_registration(
                source_down, target_down, continuous_voxel_size, transformation_matrix, use_point_to_plane
            )
            if (icp_result.inlier_rmse < inlier_rmse_best and
                    icp_result.fitness > fitness_best):
                inlier_rmse_best = icp_result.inlier_rmse
                fitness_best = icp_result.fitness
...
def main():
    ...
    output_file = "final_transformation_matrix.txt"
    final_transform = np.dot(transformation_matrix, ground_matrix_source)
    np.savetxt(output_file, final_transform)
    npy_output_file = "../../data/final_transform_matrix.npy"
    np.save(npy_output_file, final_transform)
