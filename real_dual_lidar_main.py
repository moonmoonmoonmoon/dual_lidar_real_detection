...
from src.data.dual_lidar_receiver import DualLidarReceiver
import numpy as np
import time
import logging
...
class RealTimeLidarSystem:
    def __init__(self, config: Dict):
        ...
        # 初始化各个模块
        self.receiver = DualLidarReceiver(
            config['lidar1_config'],
            config['lidar2_config']
            # config['frame_rate']
        )
        self.slam = SLAMProcessor(config.get('slam_config', {}))
        try:
            data_source = open_source(self.config['lidar1_config']['hostname'], sensor_idx=-1)
            self.metadata = data_source.metadata
            self.slam.initialize(self.metadata)
        except Exception as e:
            self.logger.error(f"SLAM初始化失败,加载元数据文件失败: {e}")
            raise
        ...
        self.target_transform_matrix = np.load(
            'data/ground_transform_target.npy')
...
def main():
    ...
    config = {
        # 双LiDAR配置
        'lidar1_config': {
            'hostname': 'os-122211001778.local',  # 第一个LiDAR的IP
            'lidar_port': 7502,
            'imu_port': 7503
        },
        'lidar2_config': {
            # 'hostname': '169.254.134.174',  # 第二个LiDAR的IP
            'hostname': 'os-122211001621.local',  # 第二个LiDAR的IP
            'lidar_port': 7504,
            'imu_port': 7503
        },
        # 'frame_rate': 10.0,
