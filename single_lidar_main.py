...
from src.processing.single_lidar_object_detector import LidarObjectDetector
...
class RealTimeLidarSystem:
    def __init__(self, config: Dict):
        ...
        try:
            data_source = open_source(config.get('pcap_path'), sensor_idx=-1)
    ...
    def process_frame(self, frame) -> Optional[Dict]:
        ...
        try:
            ...
            # print(frame.scan,type(frame.scan))
            ...
            # points_homogeneous = np.hstack((frame.points, np.ones((frame.points.shape[0], 1))))
            # points_world = (slam_result.pose_matrix @ points_homogeneous.T).T[:, :3]
            ...
            self.transformation_matrix = self.detector.transform_matrix
            pcd = pcd.transform(self.transformation_matrix)
            transformation_time = time.time() - transformation_start
            print(f"Object transformation time: {transformation_time * 1000:.1f}ms")
            # 4. 安全分析
